...
% Date: 12/19/2018
% 
function [err, FT, Fce, Fpass, Fiso, vMmax, lTtilde,lM,lMtilde,FMvtilde,vMtilde,Fpetilde] = ...
    ForceEquilibrium_FtildeState_all_tendon_M(a,fse,dfse,lMT,vMT,params,...
    Fvparam,Fpparam,Faparam,tension,aTendon,shift,m_oMFl,m_tSL,m_vmax)

FMo = ones(size(a,1),1)*params(1,:);
lMo = ones(size(a,1),1)*m_oMFl;
lTs = ones(size(a,1),1)*m_tSL;
alphao = ones(size(a,1),1)*params(2,:);
vMmax = ones(size(a,1),1)*m_vmax;
Atendonsc = aTendon;
