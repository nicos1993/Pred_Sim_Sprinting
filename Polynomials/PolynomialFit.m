% Compute polynomials to approximate muscle-tendon
% lengths, velocities and moment arms

function [muscle_spanning_joint_INFO,MuscleInfo] = PolynomialFit(MuscleData)

    % Construct polynomials for the moment arms and muscle-tendon lengths 
    
    muscle_sel=[];
    
    % Create a row vector of muscle_names indices
    ...
    % Iterate on different muscles
    for m_nr = 1:length(muscle_sel)
        
        if m_nr ~= [17,28,29,30,31,32,33]
            muscle_index = muscle_sel(m_nr);

            % Look across the different DOFs - the columns, for specific muscle
            index_dof_crossing = find(muscle_spanning_joint_INFO(muscle_index,:)==1);

            % Number of DOFs crossing
    ...
    figure();
    hold on;
    bar(categorical(MuscleData.muscle_names),lMT_all_error)
    xlimits = get(gca, 'XLim');
    plot(xlimits, [threshold, threshold], 'r', 'linewidth', 2)
    suptitle('RMS error on the approximated muscle-tendon length')
    ylabel('RMS error (m)')
    ...
    figure();
    hold on;
    bar(categorical(MuscleData.muscle_names),max(DM_all_error, [], 2))
    xlimits = get(gca, 'XLim');
    plot(xlimits, [threshold, threshold], 'r', 'linewidth', 2)
    suptitle('maximal RMS error on the approximated muscle moment arm')
    ylabel('RMS error (m)')
    ...
    figure();
    hold on;
    bar(categorical(MuscleData.muscle_names),order_all)
    ylim([0 max_order+1])
    xlimits = get(gca, 'XLim');
    plot(xlimits, [max_order, max_order], 'r', 'linewidth', 2)
    suptitle('Order of the polynomial approximation')
