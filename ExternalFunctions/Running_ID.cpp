...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < 9; ++i) ugrf[i] = g[i]; // grf controls
	...
	for (int i = 0; i < 3; i++) {
		forceGRFvec[i]  = ugrf[i];
		momentGRFvec[i] = ugrf[i + 3];
		COPvec[i] = ugrf[i + 6];
	}

	int ndof = NX / 2;

	Vec3 localCOPvec = model->getMatterSubsystem().getMobilizedBody(ground_mb).findStationLocationInAnotherBody(*state, COPvec, toesR_mb);
	...
	model->getMatterSubsystem().addInStationForce(*state, MobilizedBodyIndex(mbi_toesR), localCOPvec, forceGRFvec, groundForces);
	model->getMatterSubsystem().addInBodyTorque(*state, MobilizedBodyIndex(mbi_toesR), momentGRFvec, groundMoments);
...
int main() {
	...
	const Recorder* Recorder_arg[n_in] = { x,a, g };
