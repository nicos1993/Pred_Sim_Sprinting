...
int F_generic(const T** arg, T** res) {
	...
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 45] = value<T>(calcn_r_vG[i]); // R Calcaneus Frame Velocity in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 48] = value<T>(calcn_l_pG[i]); // L Calcaneus Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 51] = value<T>(calcn_l_vG[i]); // L Calcaneus Frame Velocity in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 54] = value<T>(toes_r_pG[i]); // R Toes Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 57] = value<T>(toes_r_vG[i]); // R Toes Frame Velocity in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 60] = value<T>(toes_l_pG[i]); // L Toes Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 63] = value<T>(toes_l_vG[i]); // L Toes Frame Velocity in Ground
