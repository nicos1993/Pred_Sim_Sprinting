...
constexpr int NU = NDOF + 18*2; // controls (accelerations + GRFs) 
...
int F_generic(const T** arg, T** res) {
	...
	// Get calcaneus Frame position & velocity in ground
	...
	// Get toes Frame position & velocity in ground
	Vec3 toes_r_pG = toes_r->getPositionInGround(*state);
	Vec3 toes_l_pG = toes_l->getPositionInGround(*state);

	SpatialVec toes_r_vG = toes_r->getVelocityInGround(*state); // SpatialVec[0] -> Angular, SpatialVec[1] -> Linear
	SpatialVec toes_l_vG = toes_l->getVelocityInGround(*state);
	...
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("toes_l").getMobilizedBodyIndex(), lSph1Pos_L_cont_adj, Vec3(ugrf[18], ugrf[19], ugrf[20]), groundForces);
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("toes_l").getMobilizedBodyIndex(), lSph2Pos_L_cont_adj, Vec3(ugrf[21], ugrf[22], ugrf[23]), groundForces);
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("calcn_l").getMobilizedBodyIndex(), lSph3Pos_L_cont_adj, Vec3(ugrf[24], ugrf[25], ugrf[26]), groundForces);
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("calcn_l").getMobilizedBodyIndex(), lSph4Pos_L_cont_adj, Vec3(ugrf[27], ugrf[28], ugrf[29]), groundForces);
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("calcn_l").getMobilizedBodyIndex(), lSph5Pos_L_cont_adj, Vec3(ugrf[30], ugrf[31], ugrf[32]), groundForces);
	model->getMatterSubsystem().addInStationForce(*state, model->getBodySet().get("calcn_l").getMobilizedBodyIndex(), lSph6Pos_L_cont_adj, Vec3(ugrf[33], ugrf[34], ugrf[35]), groundForces);
	...
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 18] = value<T>(l1Vec[1][i]);
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 21] = value<T>(l2Vec[1][i]);
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 24] = value<T>(l3Vec[1][i]);
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 27] = value<T>(l4Vec[1][i]);
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 30] = value<T>(l5Vec[1][i]);
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 33] = value<T>(l6Vec[1][i]);

	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 36] = value<T>(COM_p_G[i]);
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 39] = value<T>(COM_v_G[i]);

	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 42] = value<T>(calcn_r_pG[i]); // R Calcaneus Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 45] = value<T>(calcn_r_vG[1][i]); // R Calcaneus Frame Velocity in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 48] = value<T>(calcn_l_pG[i]); // L Calcaneus Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 51] = value<T>(calcn_l_vG[1][i]); // L Calcaneus Frame Velocity in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 54] = value<T>(toes_r_pG[i]); // R Toes Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 57] = value<T>(toes_r_vG[1][i]); // R Toes Frame Velocity in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 60] = value<T>(toes_l_pG[i]); // L Toes Frame Position in Ground
	for (int i = 0; i < 3; ++i) res[0][i + NDOF + 63] = value<T>(toes_l_vG[1][i]); // L Toes Frame Velocity in Ground

	res[0][NDOF + 66] = value<T>(calcn_mag); // distance; Calcaneus
	res[0][NDOF + 67] = value<T>(toes_mag); // distance; Toes
	res[0][NDOF + 68] = value<T>(tibia_mag); // distance; Tibia
	res[0][NDOF + 69] = value<T>(radius_mag); // distance; Radius
	res[0][NDOF + 70] = value<T>(hand_mag); // distance; Hand
	res[0][NDOF + 71] = value<T>(torsoRadius_r_mag); // distance; Torso-Radius R
	res[0][NDOF + 72] = value<T>(torsoRadius_l_mag); // distance; Torso-Radius L
	res[0][NDOF + 73] = value<T>(torsoHand_r_mag); // distance; Torso-Hand R
	res[0][NDOF + 74] = value<T>(torsoHand_l_mag); // distance; Torso-Hand L
	res[0][NDOF + 75] = value<T>(tibiaL_calcnR_mag); // distance; Tibia L - Calcaneus R
	res[0][NDOF + 76] = value<T>(tibiaR_calcnL_mag); // distance; Tibia R - Calcaneus L
	res[0][NDOF + 77] = value<T>(tibiaL_toesR_mag); // distance; Tibia L - Toes R
	res[0][NDOF + 78] = value<T>(tibiaR_toesL_mag); // distance; Tibia R - Toes L
	res[0][NDOF + 79] = value<T>(handR_pelvis_mag); // distance; Hand R - Pelvis
	res[0][NDOF + 80] = value<T>(handL_pelvis_mag); // distance; Hand L - Pelvis
	res[0][NDOF + 81] = value<T>(handR_femurR_mag); // distance; Hand R - Femur R
	res[0][NDOF + 82] = value<T>(handL_femurL_mag); // distance; Hand L - Femur L
	res[0][NDOF + 83] = value<T>(pelvisRadius_r_mag); // distance; Pelvis-Radius R
	res[0][NDOF + 84] = value<T>(pelvisRadius_l_mag); // distance; Pelvis-Radius L
...
int main() {
	...
	double res[NR + 36 + 49];
	for (int i = 0; i < NR + 36 + 49; ++i) Recorder_res[0][i] >>= res[i];
